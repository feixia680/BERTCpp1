...
TEST_F(TokenizationTest, basic_tokenizer_no_lower) {
    BasicTokenizer tokenizer(false);
...
TEST_F(TokenizationTest, wordpiece_tokenizer) {
    ...
    load_vocab("/workspace/test/test_vocab.txt", &vocab);
...
TEST_F(TokenizationTest, is_whitespace) {
    EXPECT_TRUE(_is_whitespace(' '));
    EXPECT_TRUE(_is_whitespace('\t'));
    EXPECT_TRUE(_is_whitespace('\r'));
    EXPECT_TRUE(_is_whitespace('\n'));
    EXPECT_TRUE(_is_whitespace(0x00A0));

    EXPECT_FALSE(_is_whitespace('A'));
    EXPECT_FALSE(_is_whitespace('-'));
}

TEST_F(TokenizationTest, is_control) {
    EXPECT_TRUE(_is_control(0x0005));

    EXPECT_FALSE(_is_control('A'));
    EXPECT_FALSE(_is_control(' '));
    EXPECT_FALSE(_is_control('\t'));
    EXPECT_FALSE(_is_control('\r'));
}

TEST_F(TokenizationTest, is_punctuation) {
    EXPECT_TRUE(_is_punctuation('-'));
    EXPECT_TRUE(_is_punctuation('$'));
    EXPECT_TRUE(_is_punctuation('`'));
    EXPECT_TRUE(_is_punctuation('.'));

    EXPECT_FALSE(_is_punctuation('A'));
    EXPECT_FALSE(_is_punctuation(' '));
}